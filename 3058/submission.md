...
```text
make brick 5 and 1 to make exacly the goal
Input:
one for 1
five for 5
goal for range
...
Output:
amount of one that used

Constraints:
can't loop cause time out
```
...
```text
Step 1:get input one five and goal
Step 2:turn five to five*=5
Step 3:use if five + one< goal or goal%5>one:
                print(-1)
        to find result that cant be done than print result
Step 4:use  elif five>goal and not goal%5:
                print(0)
            elif five<goal:
                print(goal-five)
            else:
                print(goal%5)
        to find one that used than print result
```
