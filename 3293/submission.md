...
```text
input: loop 5 input
output: take input crop it with *
constraints: need to delete all space that left or right side
```
...
```text
step 1:find input and variable
    dic = []
    for _ in range(5):
        text = input()
        dic.append(text)
step 2: delete all space left and right
    for i in range(5):
        while True:
            if not dic[i]:
                break
