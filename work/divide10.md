...
```text
get number between number to 0 that %10 == 0 and INCLUDE 0

Input:
number as int input

Output:
number between that number to 0 that %10 == 0 INCLUDE 0

Constraints:
need to turn int into str
```
...
```text
Step 1:get number as num int input
Step 2:make num %10 = 0 by using num -= num%10
Step 3:get result as rs = str(num)
Step 4:use while loop so i can loop process add all number like
       while num>0:
        num-=10
        rs += (f" {str(num)}")
Step 5:print(rs)
...
```text
to check if it can use with 10
```
