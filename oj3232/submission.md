...
```text
check if frog go end range if not print-1 if True print how many time it jump each jump -2 power
input : first jump range and end line
output: how many time frog jump or -1
constraints: didnt found
```
...
Step 1: get input and variable
    ...
    cnt = 0
    mix = 0
Step 2: loop frog jump till it can't or it finish
    while True:
        if mix >= goal:
            break
        if start<=0:
