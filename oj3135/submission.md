...
```text
count how many people that get the thing n as amount of people k as range of skip t as person that steal
input: n,k,t as int
output amount of people that get
```
...
Step 2: if t == 1:
                   print(count)
Step 3: loop range n to find how many ans has
    for _ in range(n):
        ppl = (ppl - 1 + k) % n + 1
        if ppl == 1:
            break
